...
abstract class OAuth2ClientBase
{
	...
	/**
	 * Сюда сохраняются дополнительные данные,
	 * которые сервера вроде ВКонтакта присылают при стандартных запросах, напр. access_token
	 * @var array
	 */
	...
	/**
	 * Получить дополнительные данные, пришедшие от сервера
	 */
	public function getResponseData()
	{
		return $this->_access_request_data;
	}
	/**
	 * Получить дополнительные данные, пришедшие от сервера
	 */
	...
	{
		return $this->_access_request_data[$key] = $value;
	}
	 ...
	 **/
	abstract public function executeRequest(OAuth2ClientRequest $req, $auth_type = '');
	/**
...
class OAuth2CurlClient extends OAuth2ClientBase
{
	...
	public function executeRequest(OAuth2ClientRequest $req, $auth_type = '')
	{
		$req->sign();

		$headers = array(
			'Authorization: ' . $req->getAuthorizationHeader($auth_type)
		);
...
class OAuth2AccessTokenObtainer
{
	 ...
	 **/
	public function webFlowRedirect(array $additional_params = array(), $method = 'get')
	{
		...
		if($method != 'post') {
			...
			$explode = explode('?', $url);
			$path = $explode[0];
			$nowww = str_replace( 'www.', '', $path );
			$domain = parse_url( $nowww );
			$host = empty($domain["host"]) ? $domain["path"] : $domain["host"];
			$data = ($explode[1]);//urlencode
			$path = str_replace('https://' . $host, '', $nowww);
	 ...
	 * @param array $additional_params Additional params for POST-request (FLG-3034)
	 **/
	public function webServerDidUserAuthorize($additional_params = array())
	{
		...
		if(is_array($additional_params) && !empty($additional_params)) {
			$params = array_merge($additional_params, $params); // FLG-3034
		}
		...
		// Этим хуком мы сохраняем дополнительные данные от сервера (вконтакт, например, пришлет user_id)
		unset($params['access_token'], $params['expires_in']);
		foreach ($params as $key => $value) {
			$this->client->setResponseData($key, $value);
		}
		return true;
	}
}
...
class OAuth2ClientRequest extends OAuth2Request
{
	 ...
	 * @param string $auth_type Token || OAuth
	 **/
	public function getAuthorizationHeader($auth_type = 'Token')
	{
		$result = $auth_type . ' ';
		...
		foreach($this->params_header as $key => $value)
		{
			...
			// FLG-3050 Тупой хук, нужно как-то переделать
			// Сервисы типа MailRu, Yandex запрашивают строку вида
			// Authorization OAuth token_here
			// Authorization Basic token_here
			if($auth_type != 'Token' && $key == 'token') {
				$result .= $value;
			} else {
